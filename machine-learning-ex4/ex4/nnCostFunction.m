...
for z = 1:m
	temp(z,:) = (-1*ymatrix(z,:)).*log(a3(:,z)').-(1.-ymatrix(z,:)).*log(1-a3(:,z)');
	sumInter = sum(temp(z,:));
	sumFinal = sumInter + sumFinal;
endfor
...
for a = 1:rows(Theta1)
	sumInter = sum(ThetaTemp1(a,2:end));
	sumFinal = sumFinal + sumInter;
endfor

ThetaTemp2 = Theta2.^2;
for b = 1:rows(Theta2)
	sumInter = sum(ThetaTemp2(b,2:end));
	sumFinal = sumFinal + sumInter;
endfor

reg = (lambda/(2*m))*sumFinal;
J = J +reg;

% Backpropagation
for t = 1:m
	delta3 = a3(:,t) - ymatrix(t,:)';
